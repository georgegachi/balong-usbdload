...
int locate_kernel(char *pbuf, uint32_t size)
{
  ...
  {
    if (strncmp(pbuf + off, "ANDROID!", 8) == 0)
      return off;
  }
  return 0;
}
...
\n",
  ...
  for (bl = 0; bl < 2; bl++)
  {
    ...
    if (bl == 0)
      continue; // do nothing else for raminit
  ...
  if (c != 0x55)
  {
    printf("\n\n!Port is not in USB Boot mode  -------------------------- USB Port Mode code: %i\n", c);
    return;
  }

  printf("\n\nModem USB Port Mode: %i \n", c);
  printf("\n========================================================================================\n");
  //return;
